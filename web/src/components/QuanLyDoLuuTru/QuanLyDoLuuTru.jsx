...
const QuanLyDoLuuTru = () => {
  ...
  const saveReservedFoods = async (foods) => {
    try {
      await axios.patch(`${API_URL}/storage/user/${userId}`, { foods }); // Use PATCH method to update the reserved foods
  ...
  const handleModalSubmit = () => {
    editForm.validateFields().then((values) => {
      const updatedFood = {
        storageDate: selectedFood.storageDate,
        outdate: selectedFood.outdate,
      ...
      saveReservedFoods(reservedFoods);
    });
  };
  ...
  return (
    <div>
      ...
      >
        {selectedFood && (
          <Form form={editForm} layout="vertical" initialValues={selectedFood}>
            <p>
              <strong>Food Name:</strong> {selectedFood.food.name}
            </p>
