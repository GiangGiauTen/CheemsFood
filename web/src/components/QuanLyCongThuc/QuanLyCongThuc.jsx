...
import { Card, Space, Modal, Button, Form, Input, Tooltip, Pagination, Row, Col } from 'antd'
...
const QuanLyCongThuc = () => {
    ...
    return (
        <div>
            ...
            <Row gutter={[16, 16]}>
                {filteredFoods.slice((currentPage - 1) * recipesPerPage, currentPage * recipesPerPage).map((recipe, id) => (
                    <Col key={id} xs={24} sm={12} md={8} lg={8}>
                        <Card
                            ...
                            actions={[
                                ...
                                <Tooltip title={favoriteRecipes.includes(recipe.recipeId) ? 'Remove from favorite' : 'Add to favorite'}>
                                    <a onClick={() => handleFavorite(recipe)}>
                                        {favoriteRecipes.includes(recipe.recipeId) ? <HeartTwoTone twoToneColor='#eb2f96' /> : <HeartTwoTone />}
                                    </a>
                                </Tooltip>
                            ]}
                        >
                            <Meta title={recipe.name} description={<div style={{ overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 3 }}>{recipe.description}</div>} />
                        </Card>
                    </Col>
                ))}
            </Row>
                ...
                {selectedFood && (
                    <Form layout='vertical'>
                        ...
                        <Form.Item label='Cách làm'>
                            <Input.TextArea value={selectedFood.description} rows={10} readOnly />
                        </Form.Item>
